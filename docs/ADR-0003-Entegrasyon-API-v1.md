...
## Status
Accepted — 2026-09-12 (A1–A3 uygulandi ve `main`'de: PR #1 A2 `c435550`, PR #2 A3 `eb5f93a`; A1 dogrudan `main`'e alindi). **A4 uygulandi** (2026-09-21, merge `1268ac5`); kapsami [ADR-0006](ADR-0006-FetchRun-Gorunurlugu-ve-Status-Ucu.md)'ya tasindi.

**A5 ikiye bolundu (2026-09-21).** **A5a uygulandi:** ornek istemci (`scripts/ornek_istemci.py`), README entegrasyon bolumu, env degiskenleri ve asagidaki upsert kuralinin duzeltilmesi — hicbir uretim kodu degismedi. **A5b (drf-spectacular semasi, `/api/v1/schema/`, `/api/v1/docs/`) henuz uygulanmadi:** v1 view'larinin tamami duz `APIView` oldugu, yanit zarfi (`results/next_cursor/has_more/count`) ile dokuz query parametresi otomatik cikarilamadigi ve yeni bagimlilik imaj rebuild'i gerektirdigi icin `@extend_schema` ile mevcut view'lara dokunmak zorunludur; bu nedenle kendi tasarim/plan dongusune birakildi.
...
- **Negative / kabul edilen sinirlar:**
  ...
  - `auto_now` icerik degismese de damgayi ilerletir; **tuketici tarafi upsert (idempotent) olmalidir**, salt insert cift kayit uretir. **Upsert anahtari `id` DEGILDIR** (bu belge 2026-09-12'de yazildiginda `id`'nin kararli oldugu sessizce varsayilmisti; [ADR-0005](ADR-0005-CVE-Saklama-ve-Gemini-Onceligi.md) bunun dogru olmadigini olctu): saklama penceresi disina dusen kayit silinir ve kaynak onu tekrar dondurdugunde yeni bir `id` ile geri gelir. Kararli anahtar `cve` bolumunde **`cve_id`**, diger bes bolumde **`link`**'tir — yazma yolunun (`news/tasks.py`, `update_or_create`) kullandigi anahtarlarla ayni. Kural A5a'da README'ye ve `scripts/ornek_istemci.py`'ye yazildi. Sinir: `NewsArticle.link` uzerinde `unique=True` yoktur (diger dort bolumde ve `CVEEntry.cve_id`'de vardir), yani `news` icin anahtar yazma yolunun sozlesmesidir, sema garantisi degildir; tuketici kendi tarafinda benzersiz indeks tanimlamalidir.
